...
APP=avidemux-qt
BIN="avidemux*qt*" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
...
DEPENDENCES="avidemux-cli flac faac lame libfdk-aac opus libogg libpulse libvorbis $kvantumver" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="libproxy libnghttp libidn libssh libpsl.so libunistring.so libxcb qt libuuid ADM_plugins libva libdrm $lib_audio_keywords $lib_browser_launcher"
...
rm -f ./*.desktop
LAUNCHER=$(grep -iRl avidemux archlinux/.junest/usr/share/applications/* | grep ".desktop" | head -1)
...
case "$1" in
	'--cli'|'cli'|'--help'|'help'|'-h') _JUNEST_CMD -- $CLI "$@";;
	''|*) _JUNEST_CMD -- $EXEC "$@";;
esac
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xapp|qt5ct|kvantum-qt5|avidemux-cli|x264|x265|flac|faac|lame|libfdk-aac|opus|libogg|libpulse|libvorbis) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
...
REPO="Avidemux-unofficial-appimage"
TAG="continuous"
VERSION="$VERSION"
