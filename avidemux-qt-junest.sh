...
APP=avidemux-qt
BIN="avidemux*qt*"
DEPENDENCES="ca-certificates avidemux-cli desktop-file-utils faac faad2 fribidi glu lame libass libdca libfdk-aac libpulse libva libvdpau libvpx libxml2 libxv opencore-amr opus qt5-base sqlite twolame x264 x265 xvidcore kvantum"
...
CLI='/usr/bin/avidemux*cli'
EXEC=$(grep -e '^Exec=.*' "${HERE}"/*.desktop | head -n 1 | cut -d "=" -f 2- | sed -e 's|%.||g')
case "$1" in
	'--cli'|'cli'|'--help'|'help'|'-h') $HERE/.local/share/junest/bin/junest -n -b "$ETC_RESOLV" 2> /dev/null -- $CLI "$@";;
